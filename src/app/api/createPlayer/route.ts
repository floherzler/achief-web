...
// export async function parsePlayerStatsTable(html: string) {
...
//       date: new Date($(cells[1]).text().trim().match(/\d{2}\.\d{2}\.\d{2}/)?.[0] || ""),
//       opponent: $(cells[2]).text().trim(),
//       points: parseInt($(cells[8]).text().trim(), 10) || 0,
//       assists: parseInt($(cells[9]).text().trim(), 10) || 0,
//       rebounds: parseReboundStats($(cells[10]).text().trim()),
//       twoPointStats: parseStatBlock($(cells[4]).text().trim()),
//       threePointStats: parseStatBlock($(cells[5]).text().trim()),
//       fgStats: parseStatBlock($(cells[6]).text().trim()),
//       ftStats: parseStatBlock($(cells[7]).text().trim()),
...
async function fetchPlayerName(html: string): Promise<string> {
  const $ = cheerio.load(html);

  // Select the <h5> element with class "av-special-heading-tag"
  const playerName = $('h5.av-special-heading-tag[itemprop="headline"]').text().trim();
  ...
  try {
    for (let year = startYear; year <= currentYear; year++) {
      ...
      if (year === currentYear) {
        ...
        return NextResponse.json(playerDoc); // Return the full player data 
      }
    }
  } catch (error) {
    console.error(error);
